...
export class AuthService {
  ...
  private async generateToken(user: JwtAuthUser) {
    ...
    const refreshToken = await this.jwtService.signAsync(
      { sub: user.id, user: { ...user, password: undefined }, token_id: uuid },
      { secret: jwtConstants.jwt_refresh_secret, expiresIn: '7d' },
    );
    ...
    return Promise.all([
      this.jwtService.signAsync(
        { sub: user.id, user: { ...user, password: undefined } },
        { secret: jwtConstants.jwt_access_secret, expiresIn: '15m' },
      ),
      refreshToken,
    ]);
  }
